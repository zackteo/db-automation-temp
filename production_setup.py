...
valid = False
while not valid:
    mon_security_group_name = input(
        "Please enter a unique name for your MongoDB security group: "
    )
    mon_des = input(
        "Please enter desired description for your new MongoDB security group: "
    )

    try:
        # try creating the group, if error, redo
        mon_secure = analytics_functions.create_security_group(
            mon_security_group_name, mon_des, mongo_permissions, ec2
        )
        valid = True
    except:
...
valid = False
while not valid:
    mys_security_group_name = input(
        "Please enter a unique name for your Mysql security group: "
    )
    mys_des = input(
        "Please enter desired description for your new Mysql security group: "
    )

    try:
        # try creating the group, if error, redo
        mys_secure = analytics_functions.create_security_group(
            mys_security_group_name, mys_des, mysql_permissions, ec2
        )
        valid = True
    except:
        print("It seems like the name is already taken, try another")
...
valid = False
while not valid:
    web_security_group_name = input(
        "Please enter a unique name for your Web Server security group: "
    )
    web_des = input(
        "Please enter desired description for your new Web Server security group: "
    )

    try:
        # try creating the group, if error, redo
        web_secure = analytics_functions.create_security_group(
            web_security_group_name, web_des, web_permissions, ec2
        )
        valid = True
    except:
        print("It seems like the name is already taken, try another")
...
while not valid:
    try:
        key_pair = input(
            "Please enter your desired new key name that does not already exist: "
        )
...
while not valid:
    ...
    while default:
        yes_no = input(
            "Would you like to use the default settings(t3a.2xlarge and ubuntu20.04) for AMI type and instance type? [y/n]: "
        )
    ...
    try:
        ### Mongo Instance
        mongo_instance = analytics_functions.create_instances(
            instance_ami, 1, tier, key_pair, mon_secure, ec2
        )
...
### Mysql Instance
mysql_instance = analytics_functions.create_instances(
    instance_ami, 1, tier, key_pair, mys_secure, ec2
)
mysql_node_id = mysql_instance[0]
mysql_node_id

### Web Server Instance
web_instance = analytics_functions.create_instances(
    instance_ami, 1, tier, key_pair, web_secure, ec2
)
...
while not success:
    try:
        ...
        c.sudo("unzip meta_kindle_store.zip")

        #        c.run(
        #            "wget https://dbds-kindle-reviews.s3-ap-southeast-1.amazonaws.com/metadata.bson"
        #        )
        # c.run(f"mongorestore --db {name_of_db} metadata.bson")
        c.run(f"mongorestore --db {name_of_db} meta_Kindle_Store.json")
...
while not success:
    try:
        ...
        c.run("mkdir data && cd data")

        c.sudo(
            "wget https://istd50043.s3-ap-southeast-1.amazonaws.com/kindle-reviews.zip"
        )
        c.sudo("apt-get install -y unzip")
        c.sudo("unzip kindle-reviews.zip")
        c.sudo("rm -rf kindle_reviews.json")
        #        c.run(
        #            "cd data && wget -c https://dbds-kindle-reviews.s3-ap-southeast-1.amazonaws.com/kindlereviews.sql"
        #        )
        ...
        #        c.run(
        #            'cd data && mysql -u ubuntu -ppassword -D dbds -e "source kindlereviews.sql"'
        #        )
        c.run(
            'cd data && mysql -u ubuntu -ppassword -D dbds -e "source kindle_reviews.csv"'
        )
...
while not success:
    try:
        ...
        c.run("git clone https://github.com/zackteo/dbfrontend-clone.git")
        # sudo apt-get update
        c.run("cd dbfrontend-clone")
        c.sudo("apt-get update")
        c.sudo("apt-get install npm -y")
        c.run("npm install")
        c.sudo("sudo apt-get install leiningen -y")

        c.run("lein do clean, cljsbuild once min")
        #        c.put(".env")
...
print(
    "All Done Setting Up The Production Site, You Can Access It By Using The Following Link: "
)
